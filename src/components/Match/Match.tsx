...
export function Match({ match }: any) {
  ...
  const [previousGameIndex, setPreviousGameIndex] = useState<number>();
  const chatData = localStorage.getItem("chat");

  useEffect(() => {
    if (previousGameIndex !== undefined && gameIndex !== previousGameIndex) {
  ...
  useEffect(() => {
    ...
    function getGameIndex(eventDetails: EventDetails): number {
      let gameIndexMatch = window.location.href.match(/game-index\/(\d+)/);
      let newGameIndex = gameIndexMatch
        ? parseInt(gameIndexMatch[1])
        : getNextUnstartedGameIndex(eventDetails);
      setGameIndex(newGameIndex);
      return newGameIndex;
    }

    function getScheduleEvent(_eventDetails: EventDetails) {
      getScheduleResponse()
        .then((response) => {
          ...
          let records =
            scheduleEvent.match.teams[0].record && scheduleEvent.match.teams[1].record
              ? [scheduleEvent.match.teams[0].record, scheduleEvent.match.teams[1].record]
              : undefined;
    ...
    function getLiveWindow(gameId: string) {
      ...
      getWindowResponse(gameId, date).then((response) => {
        ...
        const cleanSweep =
          matchEventDetails.match.games[currentGameIndex - 1].state === `completed` &&
          (matchEventDetails.match.teams[0].result.gameWins === 0 ||
            matchEventDetails.match.teams[1].result.gameWins === 0);

        const blueTeam =
          matchEventDetails &&
          matchEventDetails.match.games[currentGameIndex - 1].teams[0].id === homeTeam.id
            ? homeTeam
            : awayTeam;
        const redTeam =
          matchEventDetails &&
          matchEventDetails.match.games[currentGameIndex - 1].teams[1].id === homeTeam.id
            ? homeTeam
            : awayTeam;
        const blueTeamWonMatch =
          matchEventDetails.match.games.every(
        ...
        const blueTeamWonOnInhibitors =
          lastWindowFrame.blueTeam.inhibitors > 0 && lastWindowFrame?.redTeam.inhibitors === 0;
        const redTeamWonOnInhibitors =
          lastWindowFrame?.redTeam.inhibitors > 0 && lastWindowFrame?.blueTeam.inhibitors === 0;
        const blueTeamWon =
          matchEventDetails.match.games[currentGameIndex - 1].state === `completed` &&
          (blueTeam.result.outcome === `win` ||
            ...
            (blueTeamWonMatch &&
              currentGameIndex - 1 ===
                matchEventDetails.match.games.filter((game) => game.state === "completed").length));
        const redTeamWon =
          matchEventDetails.match.games[currentGameIndex - 1].state === `completed` &&
  ...
  function getStreamDropdown(eventDetails: EventDetails) {
    ...
    if (vods.length) {
      ...
      if (!eventDetails.streams || !eventDetails.streams.length) {
        ...
        if (
          eventDetails.match.games[streamGameIndex] &&
          eventDetails.match.games[streamGameIndex].state === "completed"
        ) {
      ...
      streamsOrVods = eventDetails.streams.sort((a, b) => {
        if (!a.coStreamer && a.mediaLocale.locale.includes(`en-`)) {
          if (a.provider === `youtube` && !b.coStreamer && b.mediaLocale.locale.includes(`en-`))
            return 1;
          return -1;
        }
        if (b.coStreamer) {
  ...
  if (
    ...
    return (
      <div className="match-container">
        <MatchDetails
          ...
          matchState={formatMatchState(eventDetails, lastWindowFrame, scheduleEvent)}
          records={records}
          results={results}
        ...
        <Game
          ...
          gameIndex={gameIndex}
          gameMetadata={metadata}
          firstWindowFrame={firstWindowFrame}
          lastDetailsFrame={lastDetailsFrame}
          lastWindowFrame={lastWindowFrame}
          outcome={currentGameOutcome}
          records={records}
          results={results}
          items={items}
          runes={runes}
    ...
    return (
      <div className="match-container">
        <MatchDetails
          eventDetails={eventDetails}
          gameMetadata={metadata}
          matchState={formatMatchState(eventDetails, firstWindowFrame, scheduleEvent)}
          records={records}
          results={results}
          scheduleEvent={scheduleEvent}
        />
    ...
    return (
      <div>
        <div className="loading-game-container">
          <div>
            ...
            <div className="live-game-card-content">
              ...
              <div className="live-game-card-team">
                {eventDetails.match.teams[1].code === "TBD" ? (
                  <TeamTBDSVG className="live-game-card-team-image" />
                ) : (
                  <img
                    className="live-game-card-team-image"
                    src={eventDetails.match.teams[1].image}
                    alt={eventDetails.match.teams[1].name}
                  />
                )}
                <span className="live-game-card-title">
                  <span>
                    <h4>{eventDetails?.match.teams[1].name}</h4>
                  </span>
                  {currentGameOutcome ? (
                    <span className="outcome">
            ...
            {scheduleEvent && eventDetails ? (
              <h3>
                ...
                {new Date(scheduleEvent.startTime).toLocaleTimeString([], {
                  year: "numeric",
                  month: "numeric",
                  day: "numeric",
                  hour: "numeric",
              ...
              <div>
                <span className="footer-notes">
                  <a
                    ...
                    href={`https://www.leagueoflegends.com/en-us/news/game-updates/patch-25-${metadata.patchVersion.split(`.`)[1].length > 1 ? metadata.patchVersion.split(`.`)[1] : "" + metadata.patchVersion.split(`.`)[1]}-notes/`}
                  >
                    Patch Version: {metadata.patchVersion}
                  </a>
                </span>
                <span className="footer-notes">
                  <button type="button" className="copy-champion-names footer-notes-link">
                    Copy Champion Names
                  </button>
                </span>
              </div>
        ...
        <script
          async
          src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-4163525631983528"
...
function getNextUnstartedGameIndex(eventDetails: EventDetails) {
  let lastCompletedGame = eventDetails.match.games
    .slice()
    .reverse()
    .find((game) => game.state === "completed");
  let nextUnstartedGame = eventDetails.match.games.find(
...
function formatMatchState(
  ...
  let gamesFinished = eventDetails.match.games.filter(
    (game) => game.state === `completed` || game.state === `unneeded`,
  );
  return gameStates[
    gamesFinished.length >= eventDetails.match.games.length ? `completed` : scheduleEvent.state
  ];
}
