...
export function ItemsDisplay({ participantId, lastFrame, items, patchVersion }: Props) {
  ...
  if (
    itemsID[0] !== undefined &&
    (itemsID[0] === 3340 || itemsID[0] === 3363 || itemsID[0] === 3364)
  ) {
...
const sortItems = (a: number, b: number) => {
  // (3364, 3363, 3340) id das wards | 3513 id do arauto
...
function formatItemDescription(item: Item) {
  ...
  return splitDescription.map((description) => {
    return <div>{description.replaceAll(/<\/\w+>/gi, ``).replaceAll(/<\w+>/gi, ``)}</div>;
  });
}
