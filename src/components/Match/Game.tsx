...
export function Game({
  ...
  const championsUrlWithPatchVersion = CHAMPIONS_URL.replace(
    `PATCH_VERSION`,
  ...
  function getStreamDropdown(eventDetails: EventDetails) {
    ...
    if (vods.length) {
      ...
      if (!eventDetails.streams || !eventDetails.streams.length) {
        if (
          eventDetails.match.games[gameIndex - 1] &&
          eventDetails.match.games[gameIndex - 1].state === "completed"
        ) {
    ...
    let dropdown = streamsOrVods.map((stream) => {
      ...
      let streamString = vods.length
        ? `VOD: ${capitalizeFirstLetter(stream.provider)}(${stream.locale})`
        : stream.coStreamer
          ? stream.mediaLocale.englishName
          : stream.provider === `twitch`
            ? `${capitalizeFirstLetter(stream.provider)}(${stream.locale}) - ${stream.parameter} - Delay: ${delayString}`
            : `${capitalizeFirstLetter(stream.provider)}(${stream.locale}) - Delay: ${delayString}`;
      return (
        <option
          value={stream.parameter}
          data-provider={stream.provider}
          data-parameter={stream.parameter}
        >
          {streamString}
        </option>
      );
    ...
    if (
      videoPlayer &&
      (!videoPlayer.hasAttribute(`added`) ||
        (vods.length && videoParameter !== streamsOrVods[0].parameter))
    ) {
    ...
    return (
      <select id="streamDropdown" className="footer-notes" onChange={handleStreamChange}>
        {dropdown}
      </select>
    );
  }
  ...
  function getVideoPlayer(newParameter?: string) {
    ...
    if (videoPlayer && !videoPlayer.hasAttribute(`added`)) {
      ...
      if (videoProvider === "youtube") {
        ...
        videoPlayer.innerHTML = `<iframe width="100%" height="100%"  frameborder="0" scrolling="no" src="https://liveshare.huya.com/iframe/lpl"></iframe>`;
      } else if (videoProvider === "afreecatv") {
        videoPlayer.innerHTML = `<iframe src="https://play.afreecatv.com/${parameter}" width="100%" height="100%" frameborder="0" allowfullscreen></iframe>`;
      }
    }
  }
  ...
  return (
    <div className="status-live-game-card">
      ...
      <div className="status-live-game-card-content">
        ...
        <div className="live-game-stats-header">
          <div className="live-game-stats-header-team-images">
            <div className="live-game-card-team">
              {blueTeam.code === "TBD" ? (
                <TeamTBDSVG className="live-game-card-team-image" />
              ) : (
                <img
                  className="live-game-card-team-image"
                  src={blueTeam.image}
                  alt={blueTeam.name}
                />
              )}
              ...
              <span className="outcome">
                {outcome ? <p className={outcome[0].outcome}>{outcome[0].outcome}</p> : null}
              </span>
            </div>
            <h1>
              <div className={`gamestate-bg-${gameState.split(` `).join(`-`)}`}>
            ...
            <div className="live-game-card-team">
              {redTeam.code === "TBD" ? (
                ...
                <img className="live-game-card-team-image" src={redTeam.image} alt={redTeam.name} />
              )}
              <span>
                <h4>{redTeam.name}</h4>
              </span>
              <span className="outcome">
                {outcome ? <p className={outcome[1].outcome}>{outcome[1].outcome}</p> : null}
              </span>
            </div>
          </div>
          ...
          <div className="live-game-stats-header-dragons">
            <div className="blue-team">
              {lastWindowFrame.blueTeam.dragons.map((dragon, i) => getDragonSVG(dragon, "blue", i))}
            </div>
            <div className="red-team">
              {lastWindowFrame.redTeam.dragons
                .slice()
                .reverse()
                .map((dragon, i) => getDragonSVG(dragon, "red", i))}
            </div>
          </div>
        </div>
          ...
          <table className="status-live-game-card-table">
            ...
            <tbody>
              {lastWindowFrame.blueTeam.participants.map((player: WindowParticipant, index) => {
                let goldDifference = getGoldDifference(
                  player,
                  "blue",
                  gameMetadata,
                  lastWindowFrame,
                ...
                return [
                  <tr
                    ...
                    key={`${gameIndex}_${championsUrlWithPatchVersion}${gameMetadata.blueTeamMetadata.participantMetadata[player.participantId - 1].championId}`}
                  >
                    <th>
                      <div className="player-champion-info">
                        ...
                        <div className="player-champion-wrapper">
                          <img
                            src={`${championsUrlWithPatchVersion}${gameMetadata.blueTeamMetadata.participantMetadata[player.participantId - 1].championId}.png`}
                            alt=""
                    ...
                    <td>
                      <div className={`player-stats player-gold-${goldDifference?.style}`}>
                        {goldDifference.goldDifference}
                      </div>
                    </td>
                  </tr>,
                  <tr
                    key={`${gameIndex}_${championsUrlWithPatchVersion}${gameMetadata.blueTeamMetadata.participantMetadata[player.participantId - 1].championId}_stats`}
                    className="champion-stats-row"
                  >
                    <td colSpan={9}>
                      <span>{getFormattedChampionStats(championDetails, runes)}</span>
                    </td>
                  </tr>,
                ];
              })}
            ...
            <tbody>
              {lastWindowFrame.redTeam.participants.map((player: WindowParticipant, index) => {
                ...
                return [
                  <tr
                    className="player-stats-row"
                    key={`${gameIndex}_${championsUrlWithPatchVersion}${gameMetadata.redTeamMetadata.participantMetadata[player.participantId - 6].championId}`}
                  >
                    <th>
                      <div className="player-champion-info">
                        <svg
                        ...
                        <div className="player-champion-wrapper">
                          <img
                            src={`${championsUrlWithPatchVersion}${gameMetadata.redTeamMetadata.participantMetadata[player.participantId - 6].championId}.png`}
                            alt=""
                            className="player-champion"
                            onError={({ currentTarget }) => {
                              currentTarget.style.display = `none`;
                  ...
                  <tr
                    key={`${gameIndex}_${championsUrlWithPatchVersion}${gameMetadata.redTeamMetadata.participantMetadata[player.participantId - 6].championId}_stats`}
                    className="champion-stats-row"
                  >
                    <td colSpan={9}>
                      <span>{getFormattedChampionStats(championDetails, runes)}</span>
        ...
        <span className="footer-notes">
          <a
            ...
            href={`https://www.leagueoflegends.com/en-us/news/game-updates/patch-26-${gameMetadata.patchVersion.split(`.`)[1].length > 1 ? gameMetadata.patchVersion.split(`.`)[1] : "" + gameMetadata.patchVersion.split(`.`)[1]}-notes/`}
          >
            Patch Version: {gameMetadata.patchVersion}
          </a>
        </span>
        <span className="footer-notes">
          <button type="button" className="copy-champion-names">
            Copy Champion Names
          </button>
        </span>
      ...
      <LiveAPIWatcher
        ...
        championsUrlWithPatchVersion={championsUrlWithPatchVersion}
        blueTeam={eventDetails.match.teams[0]}
        redTeam={eventDetails.match.teams[1]}
      />
    </div>
  );
}
  ...
  return (
    <div className={teamColor}>
      ...
      <div className="team-stats gold">
        <GoldSVG />
        <span>{Number(teamStats.totalGold).toLocaleString("en-us")}</span>
      </div>
  ...
  return (
    <div>
      ...
      <div className="footer-notes">
        Damage Share: {Math.round(championDetails.championDamageShare * 10000) / 100}%
      </div>
      <div className="footer-notes">
        Kill Participation: {Math.round(championDetails.killParticipation * 10000) / 100}%
      </div>
...
function getGoldDifference(
  player: WindowParticipant,
  side: string,
  gameMetadata: GameMetadata,
  frame: WindowFrame,
) {
  if (6 > player.participantId) {
    // blue side
...
function getDragonSVG(dragonName: string, teamColor: string, index: number) {
  ...
  switch (dragonName) {
    case "ocean":
      return <OceanDragonSVG className="dragon" key={key} />;
    case "hextech":
      return <HextechDragonSVG className="dragon" key={key} />;
    case "chemtech":
      return <ChemtechDragonSVG className="dragon" key={key} />;
    case "infernal":
      return <InfernalDragonSVG className="dragon" key={key} />;
    case "cloud":
      return <CloudDragonSVG className="dragon" key={key} />;
    case "mountain":
      return <MountainDragonSVG className="dragon" key={key} />;
    case "elder":
      return <ElderDragonSVG className="dragon" key={key} />;
  }
}
