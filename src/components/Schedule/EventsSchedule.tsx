...
function filterLiveEvents(scheduleEvent: ScheduleEvent) {
  return (
    scheduleEvent.match &&
    (scheduleEvent.state === "inProgress" ||
      (scheduleEvent.state === "unstarted" &&
        ((scheduleEvent.match.teams[0].result &&
          scheduleEvent.match.teams[0].result.gameWins > 0 &&
          !scheduleEvent.match.teams[0].result.outcome) ||
          (scheduleEvent.match.teams[0].result &&
            scheduleEvent.match.teams[1].result &&
            scheduleEvent.match.teams[1].result.gameWins > 0 &&
            !scheduleEvent.match.teams[1].result.outcome))) ||
      (scheduleEvent.state === "completed" &&
        ((scheduleEvent.match.teams[0].result &&
          scheduleEvent.match.teams[0].result.gameWins > 0 &&
          !scheduleEvent.match.teams[0].result.outcome) ||
          (scheduleEvent.match.teams[0].result &&
  ...
  if (
    ...
    (scheduleEvent.state === "inProgress" ||
      (scheduleEvent.state === "completed" &&
        scheduleEvent.match.teams[0].result &&
        scheduleEvent.match.teams[0].result.gameWins > 0) ||
      (scheduleEvent.match.teams[1].result && scheduleEvent.match.teams[1].result.gameWins > 0))
  ) {
    return;
  }
  let minDate = new Date();
